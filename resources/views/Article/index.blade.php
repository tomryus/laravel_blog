    ...
    <div class="box-body">
      <table id="example1" class="table table-bordered table-striped">
        ...
        <tr>
            ...
            <td>{{$item->title}}</td>
            <td>@if($item->picture)
                <img src={{asset('uploads/' .$item->picture)}} width="96px">
              @endif
            </td>
            ...
            <td>
                ...
                <a href="javascript:void(0)" onclick="$(this).find('form').submit()" class="btn btn-danger btn-sm">Delete
                  <form method="POST" action={{route('article.destroy',$item->id) }}  onsubmit="return confirm('Delete this category permanently?')"
                </a>
                @csrf
                @method('DELETE')
              </form> 
